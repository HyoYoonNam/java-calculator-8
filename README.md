...
- [x] 사용자는 `"//;\n1;2;3"`과 같이 `//`와 `\n` 사이에 커스텀 구분자를 지정할 수 있다.
  ...
  - [ ] `.`은 양의 실수 입력에서 필수적으로 사용되므로, 커스텀 구분자로 지정할 수 없다. 지정시 예외를 발생시킨다.
...
- [x] 커스텀 구분자가 있는 입력의 합을 리턴할 수 있다.
  - 예를 들어, `"//;\\n1,2:3;4"`에 대해 `10`을 리턴한다.
- [ ] 사용자는 양의 실수를 입력할 수 있다.
  - 예를 들어, `"1,2.3;4"`는 `10`으로 계산된다.
...
- 현재까지는 `.`이 커스텀 구분자로 지정 가능하도록 구현되어 있으니 실수 입력은 받지 않는 것이 맞다.
  - 즉, `.`을 커스텀 구분자로 지정하지 않았을 때의 `"1,2:3.4"`는 예외가 발생해야 된다.
  - `.`을 커스텀 구분자로 지정한 `"//.\\n1,2:3.4"`는 `10`으로 계산되어야 한다.
- 만약 양의 실수를 입력받을 수 있게 하려면, `.`은 구분자로 사용하지 못하도록 제한해야 한다.

~~해당 고민에 대한 답은 일단 과제 제출 후 테스트를 통해 확인하는 것이 적절하겠다.~~

지원 플랫폼에서 PR 제출 후 실행할 수 있는 예제 테스트는 기본으로 제공되는 `ApplicationTest`의 테스트 메서드들인 것으로 추정된다.
즉, 예제 테스트는 실수 입력을 받아야 되는지, 아닌지 판단할 수 있는 기준이 아니라는 것이 내 판단이다.

이 상황에 대해서는 [비슷한 상황에서 언급한 대로](#양수라는-범위에-대한-고민) 최대한 넓게 포용할 수 있도록 하겠다.
즉, 일반적으로 '양수'는 '양의 정수' 뿐만 아니라 '양의 실수'도 포함하는 의미이므로 사용자가 양의 실수를 입력할 수 있도록 하겠다.

이를 위해 다음과 같은 과정을 거친다.
1. 기존 테스트 케이스에서 커스텀 구분자로 `.`을 사용한 경우, 다른 커스텀 구분자(`-`로 통일)로 변경한다. - `separator`
2. 커스텀 구분자로 `.`을 지정하는 경우 예외를 발생시키는 테스트 메서드를 추가하고, 프로그램 로직을 구현한다. - `separator`
3. 양의 실수가 입력에 포함된 경우에 대한 테스트 메서드를 추가한다. - `separator`, `calculator`
4. 3번 테스트를 통과시키기 위해 관련된 `long[]` 타입을 모두 `double[]`로 변경한다.